...
def seminar_page(tag, lead_parts, sub, title, form, program, exit_, sched,
    ...
    plain(s, M, 12.00, CW, 0.90,
          [[('集客の逆算スケジュール　―　申込 14社 → 参加 10社 → 提携 1.5社 が目標',
...
lead(s, [('■　提携17社 × 1社あたり名簿68件 → 粗利', T_LEAD, INK),
         ('2,190万円', T_LEADEM, RED)])
...
card(s, M, 8.35, CW, 1.20, 2.40, [('KGI｜粗利額', T_LEAD, YEL)],
     [[('粗利 ＝ 獲得名簿数 1,160件 × アポ率 12％ × 商談化率 70％', T_LEAD, INK)],
      [('× 成約率 30％ × 平均単価 250万円 × 粗利率 30％ ＝ 2,190万円', T_LEAD, INK)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, head_size=T_LEAD, body_spc=1.3)
plain(s, M, 12.35, CW, CB - 12.35,
      [[('この企画で狙うのは売上', T_SUM, INK), ('7,300万円', T_SUM, RED),
        ('・粗利', T_SUM, INK), ('2,190万円', T_SUM, RED), ('。', T_SUM, INK)],
       [('その全ては「獲得名簿数」という1つの数字に依存する。', T_SUM, INK)]],
      fill=PALE, spc=1.35)
rec('起案者', '—（全数値を反映済み）')
keymsg(s, '初年度は提携17社・名簿1,160件で、粗利2,190万円を狙う。')
...
plain(s, M, 10.25, CW, CB - 10.25,
      [[('1,160件', 32, RED), ('を獲りにいく', 32, INK)]], fill=PALE)
...
plain(s, M, 5.45, CW, 3.05,
      [[('獲得名簿数 ＝ 提携社数 ', T_NUM_L, INK), ('17社', T_NUM_L, RED)],
       [('× 1社あたり名簿数 ', T_NUM_L, INK), ('68件', T_NUM_L, RED),
        ('　＝　', T_NUM_L, INK), ('1,160件', T_NUM_L, RED)]],
...
plain(s, M, CT, CW, 1.40,
      [[('KGI｜粗利　', T_LEADEM, WHITE), ('2,190万円', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
arrow(s, (W - 1.00) / 2, 5.65, 1.00, 0.80, MSO_SHAPE.DOWN_ARROW, NAVY)
plain(s, M, 6.65, CW, 1.40,
      [[('中間KPI｜獲得名簿数　', T_LEADEM, WHITE), ('1,160件', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
...
for x, hd, todo, kpi in zip(X3,
        ...
        [[[('訪問 1,080社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('6社', T_NUM_M, RED)]],
         ...
         [[('実参加 29社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('6社', T_NUM_M, RED)]]]):
...
notes(s, '施策ごとに「やること」と「追うKPI」を並べ、下段でそれが提携社数 → 名簿 → 粗利に'
         'つながることを示すページ。訪問6社＋フォーム/DL 5社＋セミナー6社＝提携17社。'
...
keymsg(s, '訪問1,080社が、提携6社と名刺864件を同時に生む。')
...
for x, hd, ls in zip(X3,
        ...
        [['・商圏3,000社を全社抽出', '・1.5ヶ月に1本のDLを年8回',
         ...
         ['・DL者と訪問名刺を積み上げ', '・期中平均695件へ週1配信',
          '・月3,000通（枠の20％）', '・セミナー集客の母数になる']]):
...
for name, kpis in groups:
    ...
    for kpi, val, basis in kpis:
        plain(s, cx14[1], y, cw14[1], rh14, [[(kpi, T_BODY, INK)]], fill=WHITE, line=NAVY,
              lw=1.0, align=PP_ALIGN.LEFT)
        plain(s, cx14[2], y, cw14[2], rh14, [[(val, T_BODY, RED)]], fill=WHITE, line=NAVY, lw=1.0)
        plain(s, cx14[3], y, cw14[3], rh14, [[(basis, T_SUB, NAVY)]],
              fill=WHITE, line=NAVY, lw=1.0, align=PP_ALIGN.LEFT)
        y += rh14
rec('経営企画', '商談率3.0％の目標設定と、名簿の共有率65％・初年度到達率35％の妥当性')
...
rows15 = [('提携社数', ('12社', '17社', '23社')),
          ('獲得名簿数', ('820件', '1,160件', '1,570件')),
          ('アポ率', ('9％', '12％', '15％')),
          ('成約率', ('22％', '30％', '35％')),
          ('契約件数', ('11件', '29件', '58件')),
          ('売上', ('2,800万円', '7,300万円', '1億4,400万円')),
          ('粗利（KGI）', ('850万円', '2,190万円', '4,330万円'))]
y = 8.55
...
keymsg(s, '投資452万円に対し、標準ケースで粗利2,190万円。')
...
for x, w, t in zip(cxR, cwR, ['年　次', '投　資', '粗利（KGI）', '回収倍率']):
    plain(s, x, 5.35, w, 0.95, [[(t, T_BODY, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
_y = 6.30
for _yr, _iv, _gp, _rt, _last in [('1年目', '452万円', '2,190万円', '4.8倍', False),
                                  ('2年目', '452万円', '4,740万円', '10.5倍', False),
                                  ('3年目', '452万円', '5,670万円', '12.5倍', False),
                                  ('3年累計', '1,356万円', '12,600万円', '9.3倍', True)]:
    _fl = PALE if _last else WHITE
...
card(s, X2[1], 10.40, W2, 1.20, CB - 11.60, [('回収の目安', T_HEAD, WHITE)],
     [[('・損益分岐は提携4社（1社あたり粗利129万円）', T_BODY, INK)],
      [('・名簿1件あたりの獲得単価は3,900円', T_BODY, INK)],
...
notes(s, '投資の内訳と回収の見通し。内訳はFB広告360万円（月30万円）、'
         ...
         '計上していない。損益分岐は提携4社（粗利2,190万円 ÷ 17社 ＝ 1社あたり129万円）。'
         '名簿1件あたりの獲得単価は452万円 ÷ 1,160件 ＝ 3,900円で、'
...
exec_page('実行｜メルマガ',
          ...
          [('配信リストをCRMに一本化する（訪問名刺・DL者）', '', '', '―'),
           ...
           ('毎週配信し、開封・クリックを記録する（月3,000通）', '', '', '4本／月'),
           ('クリックした先を週次で営業へ渡す', '', '', '4件／月')],
          '配信 3,000通　→　開封 507件（16.9％）　→　クリック 4件',
...
exec_page('実行｜セミナー',
          ...
          [('年間開催計画を確定する（年4回・日程を先に押さえる）', '船井', '9月', '―'),
           ...
           ('申込者・反応者へ架電し、参加を後押しする', '貴社', '9月', '申込14社／回'),
           ('当日運営（個別試算を提示し、決裁者同席で打診）', '貴社', '11月', '参加10社／回'),
           ('翌営業日にお礼と個別面談の日程調整を送る', '貴社', '11月', '提携1.5社／回')],
          '1回あたり 申込 14社　→　参加 10社　→　提携 1.5社（年6社）',
