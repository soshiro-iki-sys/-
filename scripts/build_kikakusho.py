...
def seminar_page(tag, lead_parts, sub, title, form, program, exit_, sched,
    ...
    plain(s, M, 12.00, CW, 0.90,
          [[('集客の逆算スケジュール　―　申込 18社 → 参加 13社 → 提携 2社 が目標',
...
lead(s, [('■　提携24社 × 1社あたり名簿68件 → 粗利', T_LEAD, INK),
         ('3,100万円', T_LEADEM, RED)])
...
card(s, M, 8.35, CW, 1.20, 2.40, [('KGI｜粗利額', T_LEAD, YEL)],
     [[('粗利 ＝ 獲得名簿数 1,640件 × アポ率 12％ × 商談化率 70％', T_LEAD, INK)],
      [('× 成約率 30％ × 平均単価 250万円 × 粗利率 30％ ＝ 3,100万円', T_LEAD, INK)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, head_size=T_LEAD, body_spc=1.3)
plain(s, M, 12.35, CW, CB - 12.35,
      [[('この企画で狙うのは売上', T_SUM, INK), ('1億300万円', T_SUM, RED),
        ('・粗利', T_SUM, INK), ('3,100万円', T_SUM, RED), ('。', T_SUM, INK)],
       [('その全ては「獲得名簿数」という1つの数字に依存する。', T_SUM, INK)]],
      fill=PALE, spc=1.35)
rec('起案者', '—（全数値を反映済み）')
keymsg(s, '初年度は提携24社・名簿1,640件で、売上1億円を狙う。')
...
plain(s, M, 10.25, CW, CB - 10.25,
      [[('1,640件', 32, RED), ('を獲りにいく', 32, INK)]], fill=PALE)
...
plain(s, M, 5.75, CW, 3.30,
      [[('獲得名簿数 ＝ 提携社数 ', T_NUM_L, INK), ('24社', T_NUM_L, RED)],
       [('× 1社あたり名簿数 ', T_NUM_L, INK), ('68件', T_NUM_L, RED),
        ('　＝　', T_NUM_L, INK), ('1,640件', T_NUM_L, RED)]],
...
plain(s, M, CT, CW, 1.40,
      [[('KGI｜粗利　', T_LEADEM, WHITE), ('3,100万円', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
arrow(s, (W - 1.00) / 2, 5.65, 1.00, 0.80, MSO_SHAPE.DOWN_ARROW, NAVY)
plain(s, M, 6.65, CW, 1.40,
      [[('中間KPI｜獲得名簿数　', T_LEADEM, WHITE), ('1,640件', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
...
for x, hd, todo, kpi in zip(X3,
        ...
        [['・4名で月30社を訪問する', '・断られても名刺を獲る',
          '・獲った名刺はメルマガへ', '・商談化した先に提携提案',
          '・年1,440社を回りきる'],
         ['・DL資料を毎月1本作る', '・フォームで3,000社に配信',
          '・FB広告でDLを訴求する', '・DL者へ週1メルマガ配信',
          '・反応者を営業へ渡す'],
         ['・オンラインで年4回開催', '・メルマガ・DMで告知する',
          '・架電で参加を後押しする', '・当日に収益試算を提示',
          '・個別面談で提携を打診']],
        [[[('訪問 1,440社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('9社', T_NUM_M, RED)]],
         [[('DL獲得 655件', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('7社', T_NUM_M, RED)]],
         [[('実参加 38社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('8社', T_NUM_M, RED)]]]):
...
notes(s, '施策ごとに「やること」と「追うKPI」を並べ、下段でそれが提携社数 → 名簿 → 粗利に'
         'つながることを示すページ。訪問9社＋フォーム/DL 7社＋セミナー8社＝提携24社。'
...
card(s, M, CT, CW, 1.30, 2.40, [('提携社数の公式', T_LEAD, YEL)],
     [[('提携社数 ＝ 訪問数 ', T_LEAD, INK), ('1,440社', T_LEAD, RED),
...
card(s, X2[0], 8.15, W2, 1.40, 3.40, [('即効獲得｜経営層に直接提示', T_HEAD, WHITE)],
     [[('・販売利益（フロー収益）', T_HEAD, INK)],
      ...
      [('・4名 × 月30社 × 12ヶ月で1,440社', T_HEAD, INK)]], body_size=T_HEAD, body_spc=1.55)
...
rec('営業', '4名体制の確保と、訪問先リスト1,440社の手当て')
keymsg(s, '訪問1,440社が、提携9社と名刺1,152件を同時に生む。')
...
card(s, M, CT, CW, 1.30, 2.40, [('DL獲得数の公式', T_LEAD, YEL)],
     [[('① フォーム送信 3,000社 × 年12回（約33,700通）→ 累積DL率 ', T_LEAD, INK),
       ('13.5％', T_LEAD, RED), ('　→　', T_LEAD, INK), ('405件', T_LEAD, RED)],
      [('② FB広告（DL訴求）月30万円 × 12ヶ月 → ', T_LEAD, INK), ('250件', T_LEAD, RED),
       ('　＝　DL獲得 ', T_LEAD, INK), ('655件', T_LEAD, RED)]],
...
for x, hd, ls in zip(X3,
        ...
        [['・商圏3,000社を全社抽出', '・毎月1本のDLを年12回',
          '・月平均2,820通で配信', '・累積DL率13.5％ → 405件'],
         ['・目的はDLのダウンロード', '・DL申込LPへ誘導する',
          '・月30万円 × 12ヶ月', '・年250件（1件 約1,440円）'],
         ['・DL者と訪問名刺を積み上げ', '・期中平均903件へ週1配信',
          '・月3,900通（枠の26％）', '・セミナー集客の母数になる']]):
...
rec('マーケ', 'フォーム送信のDL率、FB広告のDL単価、ツール枠の使用率94％の是非')
keymsg(s, '同じDL資料を、押して届け、広告で取りにいく。')
notes(s, 'DL獲得数 = フォーム送信405件 ＋ FB広告250件 ＝ 655件。'
...
card(s, M, 9.05, CW, 1.30, 2.40, [('提携社数の公式', T_LEAD, YEL)],
     [[('期中平均リスト ', T_LEAD, INK), ('903件', T_LEAD, RED),
       (' × 申込率 ', T_LEAD, INK), ('2％', T_LEAD, RED),
       (' × 参加率 ', T_LEAD, INK), ('70％', T_LEAD, RED),
       (' × 年4回', T_LEAD, INK)],
      [('→ 実参加 ', T_LEAD, INK), ('38社', T_LEAD, RED),
       ('（重複25％を控除）× 提携率 ', T_LEAD, INK), ('20％', T_LEAD, RED),
       ('　＝　', T_LEAD, INK), ('8社', T_LEAD, RED)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, head_size=T_LEAD, body_spc=1.3, lw=2.5)
plain(s, M, 13.10, CW, CB - 13.10,
      [[('前提：セミナーは', T_SUM, INK),
        ('メルマガ・DM単独でも13社集客できる企画', T_SUM, RED), ('にする。', T_SUM, INK)],
...
for name, kpis in groups:
    ...
    for kpi, val, basis in kpis:
        ...
        y += rh14
rec('経営企画', '商談率3.0％の目標設定（現状1.0％からの引き上げ幅）')
...
rows15 = [('提携社数', ('17社', '24社', '33社')),
          ('獲得名簿数', ('1,160件', '1,640件', '2,250件')),
          ('アポ率', ('9％', '12％', '15％')),
          ('成約率', ('22％', '30％', '35％')),
          ('契約件数', ('16件', '41件', '83件')),
          ('売上', ('4,000万円', '1億300万円', '2億700万円')),
          ('粗利（KGI）', ('1,200万円', '3,100万円', '6,210万円'))]
y = 8.55
...
keymsg(s, '投資452万円に対し、標準ケースで売上1億300万円。')
...
for x, w, t in zip(cxR, cwR, ['年　次', '投　資', '粗利（KGI）', '回収倍率']):
    plain(s, x, 5.35, w, 0.95, [[(t, T_BODY, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
_y = 6.30
for _yr, _iv, _gp, _rt, _last in [('1年目', '452万円', '3,100万円', '6.9倍', False),
                                  ('2年目', '452万円', '6,690万円', '14.8倍', False),
                                  ('3年目', '452万円', '8,040万円', '17.8倍', False),
                                  ('3年累計', '1,356万円', '17,830万円', '13.1倍', True)]:
    _fl = PALE if _last else WHITE
...
card(s, X2[0], 10.40, W2, 1.20, CB - 11.60, [('かからない費用', T_HEAD, WHITE)],
     [[('・DL・メルマガの制作費　当社が引き受ける', T_BODY, INK)],
      ...
      [('・訪問4名目の人件費は要確認（増員のため）', T_BODY, RED)]],
...
card(s, X2[1], 10.40, W2, 1.20, CB - 11.60, [('回収の目安', T_HEAD, WHITE)],
     [[('・損益分岐は提携4社（1社あたり粗利129万円）', T_BODY, INK)],
      [('・名簿1件あたりの獲得単価は2,800円', T_BODY, INK)],
      [('・個人向けCPA 40,000円のおよそ14分の1', T_BODY, INK)],
      [('・ツールの25％OFFが切れると年476万円', T_BODY, INK)]],
     head_size=T_HEAD, body_size=T_BODY, body_spc=1.7)
rec('経営企画', '訪問4名目の人件費の扱い、ツールの25％OFFの適用期間')
keymsg(s, '売上1億円を、投資452万円のまま取りにいく。')
notes(s, '投資の内訳と回収の見通し。内訳はFB広告360万円（月30万円）、'
         'セールスブレイカー72万円（月6万円・25％OFF適用時）、セミナー運営20万円（年4回）の'
         '合計452万円。制作費・CRM・名簿のデータ化は追加の現金支出が発生しないため計上していない。'
         'ただし訪問を3名から4名に増やすため、4名目の人件費の扱いだけは確認が必要。'
         '損益分岐は提携4社（粗利3,100万円 ÷ 24社 ＝ 1社あたり129万円）。'
         '名簿1件あたりの獲得単価は452万円 ÷ 1,640件 ＝ 2,800円で、'
         '個人向けCPA 40,000円のおよそ14分の1。'
...
exec_page('実行｜訪問',
          ...
          [('対象リストの作成（工務店・不動産会社を抽出）', '―', '済', '―'),
           ('訪問トークスクリプトの作成', '―', '済', '―'),
           ('訪問の実施（4名 ／ 1名あたり 週8社・月30社）',
            '坂元様、島様／福島様＋1名', '随時', '30社／1人'),
           ('獲得名刺のメルマガ登録', '坂元様、島様／福島様', '9月', '83社／1人'),
           ('商談化した先への提携提案・契約締結', '山﨑様', '9月', '商談3.6件／提携0.7社')],
          '訪問 120社　→　商談 3.6件　→　提携 0.7社（年9社）',
...
exec_page('実行｜フォーム・DL',
          ...
          [('リストDLで商圏3,000社を抽出し、業種で精査する', '貴社', '9月', '―'),
           ('DL資料の年間ラインナップを決める（年12本）', '船井', '9月', '―'),
           ('第1号を制作する（工務店向け・提携モデルの解説）', '船井', '10月', '毎月1本'),
           ('DL申込LPとサンクスメールを用意する（広告の受け皿）', '貴社', '10月', '―'),
           ('フォーム送信を配信する（月平均2,820通）', '貴社', '10月', '34件DL'),
           ('FB広告でDLを訴求する（月30万円・LPへ誘導）', '船井', '10月', '21件DL')],
          'フォーム 34件　＋　FB広告 21件　＝　DL 55件 /月',
...
exec_page('実行｜メルマガ',
          ...
          [('配信リストをCRMに一本化する（訪問名刺・DL者）', '', '', '―'),
           ...
           ('毎週配信し、開封・クリックを記録する（月3,900通）', '', '', '4本／月'),
           ('クリックした先を週次で営業へ渡す', '', '', '5件／月')],
          '配信 3,900通　→　開封 659件（16.9％）　→　クリック 5件',
...
exec_page('実行｜セミナー',
          ...
          [('年間開催計画を確定する（年4回・日程を先に押さえる）', '船井', '9月', '―'),
           ...
           ('申込者・反応者へ架電し、参加を後押しする', '貴社', '9月', '申込18社／回'),
           ('当日運営（個別試算を提示し、決裁者同席で打診）', '貴社', '11月', '参加13社／回'),
           ('翌営業日にお礼と個別面談の日程調整を送る', '貴社', '11月', '提携2社／回')],
          '1回あたり 申込 18社　→　参加 13社　→　提携 2社（年8社）',
...
notes(s, '本編の施策とは切り離した別軸。イー・スマイル高橋顧問（春日井市）が持つ'
         ...
         '本編の訪問1,440社とは母数も出口も別なので、同じKPI表では管理しない。'
