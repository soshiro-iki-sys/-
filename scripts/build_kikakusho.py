...
def toc():
    ...
    items = [
        ...
        ('②', 'Ⅱ. 戦略', 'センターピンはどこか', 'P5', '協議'),
        ('③', 'Ⅲ. 施策', 'どう獲り、育て、刈るか', 'P9', '協議'),
        ('④', 'Ⅳ. 数値', 'いくら投じ、いくら返るか', 'P17', '決裁'),
        ('⑤', 'Ⅴ. 実行', '誰が、いつまでに', 'P21', '協議'),
    ]
    y = 4.30
...
notes(s, '投資総額＝固定費＋変動費。「損益分岐点・回収期間」ページと数字を必ず一致させる。'
...
notes(s, '提携社数 = 総リスト数 × 反応率 × 商談化率 × 提携率。'
         'フローはメルマガ（育成）→ テレアポ（送客）→ セミナー（刈り取り）。'
...
lead(s, [('■　各施策のKPIと、その', 20, INK), ('根拠となる事例', 24, RED),
         ('を1枚にまとめる', 20, INK)])
c13 = [(0.19, 6.00), (6.19, 7.00), (13.19, 5.00), (18.19, 9.09)]
for (x, w), t in zip(c13, ['施　策', 'K P I', '仮説値', '事例｜実績値']):
...
for name, kpis in groups:
    ...
    for kpi, val in kpis:
        ...
        y += 1.00
note(s, '経営企画', '各KPIの仮説値と、事例企業の実績値')
issue(s, '論点｜', 'どの事例を仮説値の根拠として採用するか')
notes(s, '施策ごとにKPIと事例をひとまとめにした前提条件表。'
         '仮説値は事例の実績値から置く。事例企業名は確定後に差し替える（現状は〇社＠◯◯市）。'
         '主担当：経営企画。')
...
notes(s, '月次KPI目標は「KPI前提と事例」ページから逆算して設定する。主担当：起案者。')
