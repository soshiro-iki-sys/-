...
def seminar_page(tag, lead_parts, sub, title, form, program, exit_, sched,
    ...
    plain(s, M, 12.00, CW, 0.90,
          [[('集客の逆算スケジュール　―　申込 10社 → 参加 7社 → 提携 1社 が目標',
...
lead(s, [('■　提携15社 × 1社あたり名簿68件 → 粗利', T_LEAD, INK),
         ('1,930万円', T_LEADEM, RED)])
...
card(s, M, 8.35, CW, 1.20, 2.40, [('KGI｜粗利額', T_LEAD, YEL)],
     [[('粗利 ＝ 獲得名簿数 1,020件 × アポ率 12％ × 商談化率 70％', T_LEAD, INK)],
      [('× 成約率 30％ × 平均単価 250万円 × 粗利率 30％ ＝ 1,930万円', T_LEAD, INK)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, head_size=T_LEAD, body_spc=1.3)
plain(s, M, 12.35, CW, CB - 12.35,
      [[('この企画で狙うのは売上', T_SUM, INK), ('6,400万円', T_SUM, RED),
        ('・粗利', T_SUM, INK), ('1,930万円', T_SUM, RED), ('。', T_SUM, INK)],
       [('その全ては「獲得名簿数」という1つの数字に依存する。', T_SUM, INK)]],
      fill=PALE, spc=1.35)
rec('起案者', '—（全数値を反映済み）')
keymsg(s, '初年度は提携15社・名簿1,020件で、粗利1,930万円を狙う。')
...
plain(s, M, 10.25, CW, CB - 10.25,
      [[('1,020件', 32, RED), ('を獲りにいく', 32, INK)]], fill=PALE)
...
plain(s, M, 5.45, CW, 3.05,
      [[('獲得名簿数 ＝ 提携社数 ', T_NUM_L, INK), ('15社', T_NUM_L, RED)],
       [('× 1社あたり名簿数 ', T_NUM_L, INK), ('68件', T_NUM_L, RED),
        ('　＝　', T_NUM_L, INK), ('1,020件', T_NUM_L, RED)]],
...
plain(s, M, CT, CW, 1.40,
      [[('KGI｜粗利　', T_LEADEM, WHITE), ('1,930万円', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
arrow(s, (W - 1.00) / 2, 5.65, 1.00, 0.80, MSO_SHAPE.DOWN_ARROW, NAVY)
plain(s, M, 6.65, CW, 1.40,
      [[('中間KPI｜獲得名簿数　', T_LEADEM, WHITE), ('1,020件', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
...
for x, hd, todo, kpi in zip(X3,
        ...
        [[[('訪問 1,080社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('6社', T_NUM_M, RED)]],
         ...
         [[('実参加 20社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('4社', T_NUM_M, RED)]]]):
...
notes(s, '施策ごとに「やること」と「追うKPI」を並べ、下段でそれが提携社数 → 名簿 → 粗利に'
         'つながることを示すページ。訪問6社＋フォーム/DL 5社＋セミナー4社＝提携15社。'
...
card(s, X2[1], 8.15, W2, 1.40, 3.40, [('リスト補給｜担当者以上の名刺だけ', T_HEAD, WHITE)],
     [[('・受付止まりは母数に数えない', T_HEAD, INK)],
      [('・担当者クラス以上と交換（40％）', T_HEAD, INK)],
      [('・即日メルマガへ合流させる', T_HEAD, INK)]], body_size=T_HEAD, body_spc=1.55)
plain(s, M, 13.30, CW, CB - 13.30,
      [[('訪問1,080社のうち、メルマガに乗るのは', T_SUM, INK), ('432件', T_SUM, RED),
        ('。ここが母数になる。', T_SUM, INK)]], fill=PALE)
rec('営業', '訪問先リスト1,080社の確保と、名刺獲得率40％の実測')
keymsg(s, '訪問1,080社が、提携6社と名刺432件を生む。')
notes(s, '提携社数 = 訪問数 × 商談率 × 提携率。'
         'メルマガの母数になるのは担当者クラス以上と交換できた名刺だけで、'
         '受付止まりは数えない。名刺獲得率は40％で置いた（1,080社 → 432件）。'
         'ここが従来の80％前提から半減しており、セミナー参加者数に直接効く。'
         '未確定：訪問先リストの確保、商談率、名刺獲得率40％の実測。主担当：営業。')
...
for x, hd, ls in zip(X3,
        ...
        [['・商圏3,000社を全社抽出', '・1.5ヶ月に1本のDLを年8回',
         ...
         ['・DL者と訪問名刺を積み上げ', '・期中平均479件へ週1配信',
          '・月2,100通（枠の14％）', '・セミナー集客の母数になる']]):
...
card(s, M, 9.05, CW, 1.30, 2.40, [('提携社数の公式', T_LEAD, YEL)],
     [[('期中平均リスト ', T_LEAD, INK), ('479件', T_LEAD, RED),
       (' × 申込率 ', T_LEAD, INK), ('2％', T_LEAD, RED),
       (' × 参加率 ', T_LEAD, INK), ('70％', T_LEAD, RED),
       (' × 年4回', T_LEAD, INK)],
      [('→ 実参加 ', T_LEAD, INK), ('20社', T_LEAD, RED),
       ('（重複25％を控除）× 提携率 ', T_LEAD, INK), ('20％', T_LEAD, RED),
       ('　＝　', T_LEAD, INK), ('4社', T_LEAD, RED)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, head_size=T_LEAD, body_spc=1.3, lw=2.5)
plain(s, M, 13.10, CW, CB - 13.10,
      [[('前提：セミナーは', T_SUM, INK),
        ('メルマガ・DM単独でも7社集客できる企画', T_SUM, RED), ('にする。', T_SUM, INK)],
...
groups = [
    ('① 訪問・飛び込み',
     [('訪問数（年）', '1,080社', '3名 × 月30社 × 12ヶ月'),
      ...
      ('名刺獲得率', '40％', '担当者クラス以上と交換できた割合 → 432件')]),
    ('② フォーム・メルマガ・DL',
...
for name, kpis in groups:
    ...
    for kpi, val, basis in kpis:
        ...
        y += rh14
rec('経営企画', '名刺獲得率40％・商談率3.0％、名簿の共有率65％・初年度到達率35％の妥当性')
...
rows15 = [('提携社数', ('10社', '15社', '20社')),
          ('獲得名簿数', ('680件', '1,020件', '1,370件')),
          ('アポ率', ('9％', '12％', '15％')),
          ('成約率', ('22％', '30％', '35％')),
          ('契約件数', ('9件', '26件', '50件')),
          ('売上', ('2,400万円', '6,400万円', '1億2,600万円')),
          ('粗利（KGI）', ('710万円', '1,930万円', '3,780万円'))]
y = 8.55
...
keymsg(s, '投資452万円に対し、標準ケースで粗利1,930万円。')
...
for x, w, t in zip(cxR, cwR, ['年　次', '投　資', '粗利（KGI）', '回収倍率']):
    plain(s, x, 5.35, w, 0.95, [[(t, T_BODY, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
_y = 6.30
for _yr, _iv, _gp, _rt, _last in [('1年目', '452万円', '1,930万円', '4.3倍', False),
                                  ('2年目', '452万円', '4,180万円', '9.2倍', False),
                                  ('3年目', '452万円', '5,010万円', '11.1倍', False),
                                  ('3年累計', '1,356万円', '11,120万円', '8.2倍', True)]:
    _fl = PALE if _last else WHITE
...
card(s, X2[1], 10.40, W2, 1.20, CB - 11.60, [('回収の目安', T_HEAD, WHITE)],
     [[('・損益分岐は提携4社（1社あたり粗利129万円）', T_BODY, INK)],
      [('・名簿1件あたりの獲得単価は4,400円', T_BODY, INK)],
      [('・個人向けCPA 40,000円のおよそ9分の1', T_BODY, INK)],
...
notes(s, '投資の内訳と回収の見通し。内訳はFB広告360万円（月30万円）、'
         ...
         '計上していない。損益分岐は提携4社（粗利1,930万円 ÷ 15社 ＝ 1社あたり129万円）。'
         '名簿1件あたりの獲得単価は452万円 ÷ 1,020件 ＝ 4,400円で、'
         '個人向けCPA 40,000円のおよそ9分の1。'
...
exec_page('実行｜訪問',
          ...
          [('対象リストの作成（工務店・不動産会社を抽出）', '―', '済', '―'),
           ...
           ('担当者クラス以上の名刺をメルマガ登録', '坂元様、島様／福島様', '9月', '12社／1人'),
...
exec_page('実行｜メルマガ',
          ...
          [('配信リストをCRMに一本化する（訪問名刺・DL者）', '', '', '―'),
           ...
           ('毎週配信し、開封・クリックを記録する（月2,100通）', '', '', '4本／月'),
           ('クリックした先を週次で営業へ渡す', '', '', '3件／月')],
          '配信 2,100通　→　開封 355件（16.9％）　→　クリック 3件',
...
exec_page('実行｜セミナー',
          ...
          [('年間開催計画を確定する（年4回・日程を先に押さえる）', '船井', '9月', '―'),
           ...
           ('申込者・反応者へ架電し、参加を後押しする', '貴社', '9月', '申込10社／回'),
           ('当日運営（個別試算を提示し、決裁者同席で打診）', '貴社', '11月', '参加7社／回'),
           ('翌営業日にお礼と個別面談の日程調整を送る', '貴社', '11月', '提携1社／回')],
          '1回あたり 申込 10社　→　参加 7社　→　提携 1社（年4社）',
