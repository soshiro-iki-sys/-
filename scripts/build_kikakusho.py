...
def header(s, title, tag):
    ...
    tw = _probe.textlength(tag, font=_f(T_TAG)) / _PPC        # 章タグは文字幅で伸ばす
    sh = shape(s, M, 1.95, max(7.20, tw + 1.10), 0.85, fill=NAVY)
...
def seminar_page(tag, lead_parts, sub, title, form, program, exit_, sched,
                 message, note_text, pending):
    """勉強会の企画ページ。テーマ帯 → 3カード → 集客の逆算スケジュール。"""
    s = slide_blank(); header(s, 'Ⅴ. 実行', tag)
    lead(s, lead_parts)
    card(s, M, CT, CW, 1.10, 1.90, [('テ ー マ （ 案 ）　～ %s ～' % sub, T_TAG, YEL)],
         [[(title, 32, RED)]], body_fill=WHITE, head_size=T_TAG,
         body_align=PP_ALIGN.CENTER, lw=2.5)
    ...
    plain(s, M, 12.00, CW, 0.90,
          [[('集客の逆算スケジュール　―　申込 11社 → 参加 8社 → 提携 1.3社 が目標',
             T_TAG, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
    X5, W5 = cols(5, 0.28)
    for x, (hd, lines) in zip(X5, sched):
...
lead(s, [('■　提携16社 × 1社あたり名簿68件 → 粗利', T_LEAD, INK),
         ('2,060万円', T_LEADEM, RED)])
...
card(s, M, 8.35, CW, 1.20, 2.40, [('KGI｜粗利額', T_LEAD, YEL)],
     [[('粗利 ＝ 獲得名簿数 1,090件 × アポ率 12％ × 商談化率 70％', T_LEAD, INK)],
      [('× 成約率 30％ × 平均単価 250万円 × 粗利率 30％ ＝ 2,060万円', T_LEAD, INK)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, head_size=T_LEAD, body_spc=1.3)
plain(s, M, 12.35, CW, CB - 12.35,
      [[('この企画で狙うのは売上', T_SUM, INK), ('6,900万円', T_SUM, RED),
        ('・粗利', T_SUM, INK), ('2,060万円', T_SUM, RED), ('。', T_SUM, INK)],
       [('その全ては「獲得名簿数」という1つの数字に依存する。', T_SUM, INK)]],
      fill=PALE, spc=1.35)
rec('起案者', '—（全数値を反映済み）')
keymsg(s, '初年度は提携16社・名簿1,090件で、粗利2,060万円を狙う。')
...
plain(s, M, 10.25, CW, CB - 10.25,
      [[('1,090件', 32, RED), ('を獲りにいく', 32, INK)]], fill=PALE)
...
plain(s, M, CT, CW, 1.20, [[('セ ン タ ー ピ ン', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
plain(s, M, 5.45, CW, 3.05,
      [[('獲得名簿数 ＝ 提携社数 ', T_NUM_L, INK), ('16社', T_NUM_L, RED)],
       [('× 1社あたり名簿数 ', T_NUM_L, INK), ('68件', T_NUM_L, RED),
        ('　＝　', T_NUM_L, INK), ('1,090件', T_NUM_L, RED)]],
      fill=WHITE, lw=2.5, spc=1.2)
plain(s, M, 8.70, CW, 0.78,
      [[('「1社あたり名簿数 68件」の内訳　―　300件 × 65％ × 35％', T_BODY, WHITE)]],
      fill=NAVY, line=NAVY, lw=1.0)
XB, WB = cols(3, 0.90)
for x, hd, val, sub in zip(XB,
        ['① 1社あたりの保有名簿', '② 共有してもらえる名簿', '③ 初年度に預かる名簿'],
        ['300件', '195件', '68件'],
        ['年20棟 × 15年で置いた仮説', '300件のうち 65％ が共有される',
         'さらに 35％ が初年度に共有まで進む']):
    sh = shape(s, x, 9.48, WB, 2.70, fill=PALE, line=NAVY, lw=1.5)
    para(sh.text_frame, [(hd, T_SUB, NAVY)], PP_ALIGN.CENTER, 1.3, first=True)
    para(sh.text_frame, [(val, T_NUM_M, RED)], PP_ALIGN.CENTER, 1.3)
    para(sh.text_frame, [(sub, T_SUB, INK)], PP_ALIGN.CENTER, 1.3)
for x in XB[:2]:
    arrow(s, x + WB + 0.11, 10.55, 0.68, 0.56, MSO_SHAPE.RIGHT_ARROW, RED)
for x, hd, lines in zip(X3,
                        ['① 目的を一本化', '② 提携後は必ず回収', '③ 他のKPIは追わない'],
                        [['訪問もDLもメルマガも、', '目的は提携社数'],
                         ['提携＝ゴールではない。', '名簿の回収までが1件'],
                         ['名簿数に効かない施策は、', 'やらないと決める']]):
    card(s, x, 12.45, W3, 1.05, CB - 13.50, [(hd, T_HEAD, WHITE)],
         [[(t, T_BODY, INK)] for t in lines], body_align=PP_ALIGN.CENTER, body_spc=1.5)
rec('起案者', '保有名簿300件・共有率65％・初年度到達率35％の3つの妥当性')
keymsg(s, '追う数字は「獲得名簿数」ただ1つ。')
notes(s, '獲得名簿数 = 提携社数 × 1社あたり名簿数。'
         '1社あたり名簿数68件は3段のかけ算で置いている。'
         '①保有名簿300件＝年20棟 × 15年（実データではなく仮説）。'
         '②そのうち65％＝195件が実際に共有される（すでに他社が入っている先、'
         '連絡が取れない先、施主が嫌がる先を落とした残り）。'
         '③さらに初年度に名簿共有まで進むのは提携先の35％。'
         '1件成約して品質を見てもらう → 案件ごとの紹介 → 名簿共有、の3段階を想定している。'
         '68件は特定の1社から受け取る件数ではなく、名簿を出さない先も含めた全提携先の平均値。'
         '②と③はどちらも推定なので、実感値があれば置き換える。主担当：起案者。')
...
plain(s, M, CT, CW, 1.40,
      [[('KGI｜粗利　', T_LEADEM, WHITE), ('2,060万円', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
arrow(s, (W - 1.00) / 2, 5.65, 1.00, 0.80, MSO_SHAPE.DOWN_ARROW, NAVY)
plain(s, M, 6.65, CW, 1.40,
      [[('中間KPI｜獲得名簿数　', T_LEADEM, WHITE), ('1,090件', T_LEADEM, YEL)]],
      fill=NAVY, line=NAVY, lw=1.5)
...
keymsg(s, '各施策に取り組むことで、KGI粗利を目指す。')
notes(s, 'KGI（粗利）→ 中間KPI（獲得名簿数）→ 施策KPI の3階層。'
         '各率は「KPI前提と事例」ページと一致させる。主担当：経営企画。')

# --- 7 施策別のやることとKPI ---
s = slide_blank(); header(s, 'Ⅱ. 戦略', '施策別のやることとKPI')
lead(s, [('■　施策ごとの', T_LEAD, INK), ('実施事項', T_LEADEM, RED), ('と', T_LEAD, INK),
         ('追うKPI', T_LEADEM, RED)])
for x, hd, todo, kpi in zip(X3,
        ['① 訪問・飛び込み', '② フォーム・メルマガ・DL', '③ セミナー（年4回）'],
        [['・3名で月30社を訪問する', '・担当者クラス以上と名刺交換',
          '・交換できた名刺をメルマガへ', '・商談化した先に提携提案',
          '・年1,080社を回りきる'],
         ['・DL資料を1.5ヶ月に1本作る', '・フォームで3,000社に配信',
          '・FB広告でDLを訴求する', '・DL者へ週1メルマガ配信',
          '・反応者を営業へ渡す'],
         ['・オンラインで年4回開催', '・メルマガ・DMで告知する',
          '・架電で参加を後押しする', '・当日に収益試算を提示',
          '・個別面談で提携を打診']],
        [[[('訪問 1,080社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('6社', T_NUM_M, RED)]],
         [[('DL獲得 526件', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('5社', T_NUM_M, RED)]],
         [[('実参加 22社', T_BODY, INK)], [('→　提携 ', T_BODY, INK), ('5社', T_NUM_M, RED)]]]):
...
card(s, X2[1], 8.15, W2, 1.40, 3.40, [('リスト補給｜担当者以上の名刺だけ', T_HEAD, WHITE)],
     [[('・受付止まりは母数に数えない', T_HEAD, INK)],
      [('・担当者クラス以上と交換（50％）', T_HEAD, INK)],
      [('・即日メルマガへ合流させる', T_HEAD, INK)]], body_size=T_HEAD, body_spc=1.55)
plain(s, M, 13.30, CW, CB - 13.30,
      [[('訪問1,080社のうち、メルマガに乗るのは', T_SUM, INK), ('540件', T_SUM, RED),
        ('。ここが母数になる。', T_SUM, INK)]], fill=PALE)
rec('営業', '訪問先リスト1,080社の確保と、名刺獲得率50％の実測')
keymsg(s, '訪問1,080社が、提携6社と名刺540件を生む。')
notes(s, '提携社数 = 訪問数 × 商談率 × 提携率。'
         'メルマガの母数になるのは担当者クラス以上と交換できた名刺だけで、'
         '受付止まりは数えない。名刺獲得率は50％で置いた（1,080社 → 540件）。'
         '2社に1社は担当者クラス以上と交換できる、という置き方。'
         'ここはセミナー参加者数に直接効くので、実測して差し替えたい。'
         '未確定：訪問先リストの確保、商談率、名刺獲得率50％の実測。主担当：営業。')
...
card(s, M, CT, CW, 1.30, 2.40, [('DL獲得数の公式', T_LEAD, YEL)],
     [[('① フォーム送信 3,000社 × 年8回（約23,000通）→ 累積DL率 ', T_LEAD, INK),
       ('9.2％', T_LEAD, RED), ('　→　', T_LEAD, INK), ('276件', T_LEAD, RED)],
      [('② FB広告（DL訴求）月30万円 × 12ヶ月 → ', T_LEAD, INK), ('250件', T_LEAD, RED),
       ('　＝　DL獲得 ', T_LEAD, INK), ('526件', T_LEAD, RED)]],
...
for x, hd, ls in zip(X3,
        ...
        [['・商圏3,000社を全社抽出', '・1.5ヶ月に1本のDLを年8回',
          '・月平均1,920通で配信', '・累積DL率9.2％ → 276件'],
         ['・目的はDLのダウンロード', '・DL申込LPへ誘導する',
          '・月30万円 × 12ヶ月', '・年250件（1件 約1,440円）'],
         ['・DL者と訪問名刺を積み上げ', '・期中平均533件へ週1配信',
          '・月2,300通（枠の15％）', '・セミナー集客の母数になる']]):
...
notes(s, 'DL獲得数 = フォーム送信276件 ＋ FB広告250件 ＝ 526件。'
...
card(s, M, 9.05, CW, 1.30, 2.40, [('提携社数の公式', T_LEAD, YEL)],
     [[('期中平均リスト ', T_LEAD, INK), ('533件', T_LEAD, RED),
       (' × 申込率 ', T_LEAD, INK), ('2％', T_LEAD, RED),
       (' × 参加率 ', T_LEAD, INK), ('70％', T_LEAD, RED),
       (' × 年4回', T_LEAD, INK)],
      [('→ 実参加 ', T_LEAD, INK), ('22社', T_LEAD, RED),
       ('（重複25％を控除）× 提携率 ', T_LEAD, INK), ('20％', T_LEAD, RED),
       ('　＝　', T_LEAD, INK), ('5社', T_LEAD, RED)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, head_size=T_LEAD, body_spc=1.3, lw=2.5)
plain(s, M, 13.10, CW, CB - 13.10,
      [[('前提：セミナーは', T_SUM, INK),
        ('メルマガ・DM単独でも8社集客できる企画', T_SUM, RED), ('にする。', T_SUM, INK)],
...
groups = [
    ('① 訪問・飛び込み',
     [('訪問数（年）', '1,080社', '3名 × 月30社 × 12ヶ月'),
      ('商談率', '3.0％', '目標値（現状1.0％からの引き上げ）'),
      ('提携率', '20％', 'T社＠山梨県：商談15件 → 提携3社'),
      ('名刺獲得率', '50％', '担当者クラス以上と交換できた割合 → 540件')]),
    ('② フォーム・メルマガ・DL',
     [('フォーム配信母数', '3,000社', '商圏の全社（訪問先も除外しない）'),
      ('累積DL率（年8回）', '9.2％', '276件がDL。月1,920通＝プラン枠の64％'),
      ('メルマガ開封率', '16.9％', '船井総研：7,775通 → 1,313件'),
      ('FB広告からのDL', '250件', '月30万円 × 12ヶ月。DL申込LPへ誘導')]),
    ('③ セミナー',
     [('セミナー申込率', '2％', '期中平均リスト533件 → 参加8社/回'),
      ('参加者からの提携率', '20％', '訪問の提携率と同水準で設定')]),
    ('④ 名簿の前提\n（掛け合わせて68件）',
     [('1社あたりの保有名簿', '300件', '年20棟 × 15年で置いた仮説'),
      ('名簿の共有率', '65％', '300件のうち実際に共有される割合 → 195件'),
      ('初年度の共有到達率', '35％', '提携から名簿共有まで進む先の割合')]),
]
y = CT + 1.00
rh14 = (CB - y) / sum(len(g[1]) for g in groups)
for name, kpis in groups:
    plain(s, cx14[0], y, cw14[0], rh14 * len(kpis),
          [[(t, T_BODY, NAVY)] for t in name.split('\n')],
          fill=PALE, line=NAVY, lw=1.0, spc=1.25)
    for kpi, val, basis in kpis:
        plain(s, cx14[1], y, cw14[1], rh14, [[(kpi, T_SUB, INK)]], fill=WHITE, line=NAVY,
              lw=1.0, align=PP_ALIGN.LEFT)
        plain(s, cx14[2], y, cw14[2], rh14, [[(val, T_SUB, RED)]], fill=WHITE, line=NAVY, lw=1.0)
        plain(s, cx14[3], y, cw14[3], rh14, [[(basis, T_SUB, NAVY)]],
              fill=WHITE, line=NAVY, lw=1.0, align=PP_ALIGN.LEFT)
        y += rh14
rec('経営企画', '名刺獲得率50％・商談率3.0％、名簿の共有率65％・初年度到達率35％の妥当性')
keymsg(s, '仮説値はすべて、事例の実績値から置いている。')
notes(s, '施策ごとにKPIと事例をひとまとめにした前提条件表。仮説値は事例の実績値から置く。'
         '④の3行（保有名簿300件・共有率65％・初年度到達率35％）は実データではなく推定で、'
         'この3つのかけ算が「1社あたり名簿数68件」になる。'
         'ここが動くと獲得名簿数がそのまま比例して動くため、最も検証したい前提。主担当：経営企画。')
...
rows15 = [('提携社数', ('11社', '16社', '22社')),
          ('獲得名簿数', ('750件', '1,090件', '1,500件')),
          ('アポ率', ('9％', '12％', '15％')),
          ('成約率', ('22％', '30％', '35％')),
          ('契約件数', ('10件', '28件', '55件')),
          ('売上', ('2,600万円', '6,900万円', '1億3,800万円')),
          ('粗利（KGI）', ('780万円', '2,060万円', '4,140万円'))]
y = 8.55
...
keymsg(s, '投資452万円に対し、標準ケースで粗利2,060万円。')
...
for _hd, _rows in [
    ('STEP 1｜提携社数はどう出したか',
     [[('訪問 ', T_TAG, INK), ('1,080社', T_TAG, RED), (' × 商談率 ', T_TAG, INK),
       ('3.0％', T_TAG, RED), (' × 提携率 ', T_TAG, INK), ('20％', T_TAG, RED),
       ('　＝　訪問から ', T_TAG, INK), ('6社', T_TAG, RED)],
      [('＋ DL獲得 526件 → ', T_TAG, INK), ('5社', T_TAG, RED),
       ('　＋ セミナー実参加 22社 × 20％ → ', T_TAG, INK), ('5社', T_TAG, RED),
       ('　＝　', T_TAG, INK), ('16社', T_TAG, RED)]]),
    ('STEP 2｜獲得名簿数はどう出したか',
     [[('1社あたり保有名簿 ', T_TAG, INK), ('300件', T_TAG, RED),
       (' × 共有される割合 ', T_TAG, INK), ('65％', T_TAG, RED),
       ('　＝　1社あたり ', T_TAG, INK), ('195件', T_TAG, RED)],
      [('提携 16社 × 初年度に共有まで進む率 ', T_TAG, INK), ('35％', T_TAG, RED),
       (' ＝ 6社　→　6社 × 195件 ＝ ', T_TAG, INK), ('1,090件', T_TAG, RED)]]),
    ('STEP 3｜売上・粗利はどう出したか',
     [[('名簿 1,090件 × アポ率 ', T_TAG, INK), ('12％', T_TAG, RED),
       (' × 商談化率 ', T_TAG, INK), ('70％', T_TAG, RED),
...
lead(s, [('■　この', T_LEAD, INK), ('3STEP', T_LEADEM, RED),
         ('で、提携が生まれる状態をつくる', T_LEAD, INK)])
XR, WR = cols(3, 0.85)
for x, hd, lines in zip(XR, ['S T E P 1', 'S T E P 2', 'S T E P 3'],
...
exec_page('実行｜訪問',
          ...
          [('対象リストの作成（工務店・不動産会社を抽出）', '―', '済', '―'),
           ...
           ('担当者クラス以上の名刺をメルマガ登録', '坂元様、島様／福島様', '9月', '15社／1人'),
           ('商談化した先への提携提案・契約締結', '山﨑様', '9月', '商談2.7件／提携0.5社')],
          '訪問 90社　→　商談 2.7件　→　提携 0.5社（年6社）',
          '訪問によりリード獲得＋最短の提携を拾いに行く。', '営業')
...
exec_page('実行｜フォーム・DL',
          ...
          [('リストDLで商圏3,000社を抽出し、業種で精査する', '貴社', '9月', '―'),
           ('DL資料の年間ラインナップを決める（年8本）', '船井', '9月', '―'),
           ('第1号を制作する（工務店向け・提携モデルの解説）', '船井', '10月', '1.5ヶ月に1本'),
           ('DL申込LPとサンクスメールを用意する（広告の受け皿）', '貴社', '10月', '―'),
           ('フォーム送信を配信する（月平均1,920通）', '貴社', '10月', '23件DL'),
           ('FB広告でDLを訴求する（月30万円・LPへ誘導）', '船井', '10月', '21件DL')],
          'フォーム 23件　＋　FB広告 21件　＝　DL 44件 /月',
...
exec_page('実行｜メルマガ',
          ...
          [('配信リストをCRMに一本化する（訪問名刺・DL者）', '', '', '―'),
           ...
           ('毎週配信し、開封・クリックを記録する（月2,300通）', '', '', '4本／月'),
           ('クリックした先を週次で営業へ渡す', '', '', '3件／月')],
          '配信 2,300通　→　開封 389件（16.9％）　→　クリック 3件',
...
exec_page('実行｜セミナー',
          ...
          [('年間開催計画を確定する（年4回・日程を先に押さえる）', '船井', '9月', '―'),
           ...
           ('申込者・反応者へ架電し、参加を後押しする', '貴社', '9月', '申込11社／回'),
           ('当日運営（個別試算を提示し、決裁者同席で打診）', '貴社', '11月', '参加8社／回'),
           ('翌営業日にお礼と個別面談の日程調整を送る', '貴社', '11月', '提携1.3社／回')],
          '1回あたり 申込 11社　→　参加 8社　→　提携 1.3社（年5社）',
          'セミナーは四半期に1回。開催日から逆算して動く。', 'マーケ／営業')

# --- 25 勉強会テーマ案 ---
s = slide_blank(); header(s, 'Ⅴ. 実行', '勉強会テーマ案')
lead(s, [('■　', T_LEAD, INK), ('相手が変われば、刺さるテーマも変わる', T_LEADEM, RED),
         ('。2パターンで用意する', T_LEAD, INK)])
for x, hd, lines in zip(X2,
        ['案A｜工務店・ビルダー', '案B｜不動産会社'],
        [[('【対　象】', NAVY), ('　新築を手がける工務店', INK),
          ('【テーマ】', NAVY), ('　太陽光・蓄電池 収益化セミナー', INK),
          ('【訴　求】', NAVY), ('　工事を持たずに1棟あたりの粗利を上げる', INK),
    ...
    sh = shape(s, x, CT, W2, 1.30, fill=NAVY, line=NAVY, lw=1.5)
    para(sh.text_frame, [(hd, T_HEAD, WHITE)], PP_ALIGN.CENTER, first=True)
    bullets(s, x, 5.35, W2, 7.60, [[(t, T_BODY, c)] for t, c in lines], spc=1.6)
plain(s, M, 13.30, CW, CB - 13.30,
...
keymsg(s, '工務店だけでなく不動産会社も視野にアプローチしていく。')
notes(s, '勉強会のテーマ案を対象業種ごとに2パターン用意したページ。'
         '案Aは工務店・ビルダー（施主名簿）、案Bは不動産会社（オーナー名簿・件数が多い）。'
         '次回は案Aで実施し、反応を見て案Bへ広げる想定。主担当：マーケ／営業。')

# --- 26 勉強会企画｜案A 工務店 ---
seminar_page('勉強会企画｜案A 工務店・ビルダー',
    [('■　対象は', T_LEAD, INK), ('新築を手がける工務店', T_LEADEM, RED),
     ('。工事を持たずに1棟の粗利を上げる', T_LEAD, INK)],
    '提携手数料で売上をつくる', '工務店向け　太陽光・蓄電池で収益化セミナー',
    ['・Zoomウェビナー（90分）', '・対象 工務店・ビルダー',
     '・定員 30社（1社2名まで）', '・参加費 無料', '・開催日【　　月　　日】'],
    ['・第1部 市場と補助金の最新', '・第2部 提携モデルの解説',
     '・第3部 収益試算のデモ', '・第4部 質疑・個別相談の案内'],
    ['・終了時に個別面談を案内', '・後日個別で試算を提示',
     '・決裁者同席で提携を打診', '・参加8社 → 提携1.3社が目標'],
    [('1.5ヶ月前｜告知', ['・メルマガで初告知', '・DMを発送する']),
     ('1ヶ月前｜追込み', ['・反応者へ架電', '・メルマガ2回目']),
     ('2週間前｜確定', ['・受講URLを案内', '・欠席防止の連絡']),
     ('前日｜最終確認', ['・参加者リスト確定', '・個別提案先を選ぶ']),
     ('当日｜刈り取り', ['・収益試算を提示', '・個別面談へ誘導'])],
    '太陽光・蓄電池に絞り、オンラインで集め、提携で締める。',
    '案Aの企画シート。対象は新築を手がける工務店で、獲得できるのは施主名簿。'
    'テーマは太陽光・蓄電池に特化し、オンライン（Zoomウェビナー90分）で開催、出口は提携。'
    'オンラインのため当日その場での締結はできないので、終了時に個別面談を案内し、'
    '後日個別試算を提示して決裁者同席で打診する。'
    'T社＠山梨県の事例のとおり、集客は遅くとも1ヶ月半前から動かす。まずはこの案で1回目を実施する。',
    '開催日時、登壇者、DM発送先')

# --- 27 勉強会企画｜案B 不動産 ---
seminar_page('勉強会企画｜案B 不動産会社',
    [('■　対象は', T_LEAD, INK), ('売買仲介・賃貸管理会社', T_LEADEM, RED),
     ('。オーナー名簿がそのまま母数になる', T_LEAD, INK)],
    '管理手数料以外の収益源をつくる', '所有物件の価値を上げる 太陽光活用セミナー',
    ['・Zoomウェビナー（90分）', '・対象 売買仲介・賃貸管理',
     '・定員 30社（1社2名まで）', '・参加費 無料', '・開催日【　　月　　日】'],
    ['・第1部 電気代高騰とオーナーの関心', '・第2部 物件価値と入居率への効果',
     '・第3部 オーナー紹介モデルの解説', '・第4部 収益試算と個別相談の案内'],
    ['・終了時に個別面談を案内', '・オーナー名簿の共有範囲を確認',
     '・管理会社の決裁者へ打診', '・参加8社 → 提携1.3社が目標'],
    [('1.5ヶ月前｜告知', ['・不動産向けリストへDM', '・メルマガで初告知']),
     ('1ヶ月前｜追込み', ['・反応者へ架電', '・メルマガ2回目']),
     ('2週間前｜確定', ['・受講URLを案内', '・欠席防止の連絡']),
     ('前日｜最終確認', ['・参加者リスト確定', '・管理戸数を事前確認']),
     ('当日｜刈り取り', ['・オーナー向け試算を提示', '・個別面談へ誘導'])],
    '不動産は、オーナー名簿の件数がそのまま伸びしろになる。',
    '案Bの企画シート。対象は売買仲介・賃貸管理会社で、獲得できるのはオーナー名簿。'
    '名簿の件数では案Aより大きく、名簿獲得という目的では最も効く。'
    '訴求は「管理手数料以外の収益源をつくる」。当日は物件価値と入居率への効果を示し、'
    'オーナーへの案内という形で名簿の共有範囲を確認する。'
    '2案のうち名簿の件数では案Bが最も大きい。1回目（案A）の反応を見てから実施する想定。',
    '開催日時、登壇者、不動産向けDMリストの入手')
