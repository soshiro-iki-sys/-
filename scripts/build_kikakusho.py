...
def toc():
    ...
    items = [
        ('①', 'Ⅰ. 結論', '決めていただきたいこと', 'P3'),
        ('②', 'Ⅱ. 戦略', 'センターピンはどこか', 'P4'),
        ('③', 'Ⅲ. 施策', 'どこから獲り、どう育てるか', 'P8'),
        ('④', 'Ⅳ. 数値', 'いくら投じ、いくら返るか', 'P16'),
        ('⑤', 'Ⅴ. 実行', '誰が、いつまでに', 'P19'),
    ]
    y = 4.30
...
card(s, LM, 8.75, CW, 1.30, 2.40, [('KGI｜粗利額', 20, YEL)],
     [[('粗利 ＝ 総名簿数 1,755件 × アポ率 12％ × 商談化率 70％', 20, INK)],
      [('× 成約率 30％ × 平均単価 250万円 × 粗利率 25％ ＝ 2,760万円', 20, INK)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, body_spc=1.2)
plain(s, LM, 12.55, CW, 2.65,
      [[('この企画で狙うのは粗利', 22, INK), ('2,760万円', 22, RED), ('。', 22, INK)],
...
plain(s, LM, 4.05, CW, 1.50,
      [[('KGI｜粗利　', 24, WHITE), ('2,760万円', 24, YEL)]], fill=NAVY, line=NAVY, lw=1.5)
...
# --- 8 提携ルートの現在地 ---
s = slide_blank(); header(s, 'Ⅲ. 施策', '提携ルートの現在地')
lead(s, [('■　提携の入口は、すでに', 20, INK), ('3つ', 24, RED), ('立ち上がっている', 20, INK)])
for x, hd, lines in [
        (LM, 'ルートA｜TRENDE社',
         ['・伊藤忠商事の持分法適用会社', '・蓄電池のリース／リプレイス',
          '・全国に家庭顧客 約13,800件', '・業務提携を協議中（次ページ）']),
        (9.47, 'ルートB｜イー・スマイル',
         ['・高橋顧問（春日井市）経由', '・東海圏の工務店・設備',
          '・パイプ 5,000社', '・ここから点検先を開拓する']),
        (18.74, 'ルートC｜自社訪問',
         ['・春日井市事務所を拠点', '・自社3名（坂元・田中・島）',
          '・金曜を除く平日16:00〜', '・年1,080社を訪問する'])]:
    card(s, x, 4.05, 8.54, 1.50, 5.30, [(hd, 18, WHITE)],
         [[(t, 14, INK)] for t in lines], body_align=PP_ALIGN.LEFT, body_spc=1.45,
         body_size=14)
plain(s, LM, 11.15, CW, 4.05,
      [[('本計画の提携', 22, INK), ('26社', 22, RED),
        ('は、ルートC（自社訪問）を土台に置いた数字。', 22, INK)],
       [('ルートA・Bは母数の桁が違うため、', 22, INK),
        ('計画には織り込まず上振れ要因として別枠管理', 22, RED), ('する。', 22, INK)]],
      fill=PALE, spc=1.35)
rec('営業', 'ルートA・Bを計画に織り込むか、別枠のままとするか')
issue(s, '論点｜', 'A・Bを初年度計画に織り込むか')
notes(s, '提携の入口は3ルート。A（TRENDE）とB（イー・スマイル経由の東海圏5,000社）は'
         '母数が大きく確度が読めないため、初年度計画の26社にはC（自社訪問）ベースの数字だけを'
         '置き、A・Bは上振れ要因として別枠で管理する。主担当：営業。')

# --- 9 提携先｜TRENDE社 ---
s = slide_blank(); header(s, 'Ⅲ. 施策', '提携先｜TRENDE社')
lead(s, [('■　伊藤忠商事の持分法適用会社。全国に', 20, INK),
         ('約13,800件', 24, RED), ('の家庭顧客基盤', 20, INK)])
...
issue(s, '論点｜', '案①・②・③のどれで進めるか')
notes(s, 'TRENDE社（伊藤忠商事の持分法適用会社）との業務提携の検討ページ。'
...
card(s, 0.29, 7.95, 13.17, 1.40, 3.45, [('体制｜春日井市事務所を拠点に', 20, WHITE)],
     [[('・自社3名（坂元・田中・島）が稼働中', 17, INK)],
      [('・金曜を除く平日16:00〜、高橋顧問と同行', 17, INK)],
      [('・3名 × 月30社 × 12ヶ月で1,080社', 17, INK)]], body_size=17)
card(s, 14.06, 7.95, 13.17, 1.40, 3.45, [('リスト｜東海圏5,000社のパイプ', 20, WHITE)],
     [[('・イー・スマイル高橋顧問経由で確保', 17, INK)],
      [('・東海圏の工務店・設備 5,000社', 17, INK)],
      [('・断られても名刺を獲り、即日メルマガへ', 17, INK)]], body_size=17)
plain(s, LM, 13.00, CW, 2.20,
      [[('訪問の成果はゼロにならない。', 24, INK), ('断り ＝ リード獲得', 24, RED),
        ('と定義する。', 24, INK)]], fill=PALE)
note(s, '営業', '5,000社パイプからの訪問先の切り出し方')
issue(s, '論点｜', '月次の訪問数目標と、担当の割り当て')
notes(s, '提携社数 = 訪問数 × 商談率 × 提携率。訪問リストはイー・スマイル高橋顧問（春日井市）'
         'が持つ東海圏の工務店・設備5,000社のパイプから切り出す。'
         '未確定：5,000社のうち訪問可能な社数、商談率。主担当：営業。')
...
issue(s, '論点｜', 'フォーム配信の頻度と本数をどこまで増やすか')
...
issue(s, '論点｜', '1回あたりの集客目標と、年4回でよいか')
...
groups = [
    ('① 訪問・飛び込み',
     [('訪問先パイプ', '5,000社', '東海圏の工務店・設備（高橋顧問のパイプ）'),
...
for a, vals in [('提携社数', ('17社', '26社', '35社')),
                ...
                ('粗利（KGI）', ('1,000万円', '2,760万円', '5,420万円'))]:
...
exec_page('実行｜訪問',
          [('■　訪問を、', 20, INK), ('いつ・誰が・何件やるか', 24, RED),
           ('まで落とす', 20, INK)],
          ['高橋顧問の5,000社パイプから訪問対象を切り出す',
           '訪問トークスクリプトの作成',
           '訪問の実施（3名 × 月30社／金曜を除く平日16:00〜）',
           '獲得名刺のメルマガ登録',
           '商談化した先への提携提案・契約締結'],
          '訪問 90社　→　商談 7件　→　提携 1.3社（年16社）',
          '5,000社のうち、まずどこから当たるか', '営業')
