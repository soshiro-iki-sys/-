...
def cover():
    ...
    para(tf, [('[ 企画書｜協議用ドラフト ]　', 28, WHITE)], PP_ALIGN.LEFT, first=True)
    ...
    tf = textbox(s, 8.89, 10.75, 17.64, 1.20, MSO_ANCHOR.TOP)
    para(tf, [('◯◯会議　2026年◯月◯日　Ver.0.1', 20, WHITE)], PP_ALIGN.LEFT, first=True)
    ...
    notes(s, '打合せ資料の表紙。会議名・開催日・版数・作成者は確定後に差し替える。')
    return s
...
def toc():
    ...
    items = [
        ('①', 'Ⅰ. 結論', '決めていただきたいこと', 'P4', '決裁'),
        ('②', 'Ⅱ. 戦略', 'センターピンはどこか', 'P6', '協議'),
        ('③', 'Ⅲ. 施策', 'どう獲り、育て、刈るか', 'P10', '協議'),
        ('④', 'Ⅳ. 数値', 'いくら投じ、いくら返るか', 'P14', '決裁'),
        ('⑤', 'Ⅴ. 実行', '誰が、いつまでに', 'P18', '協議'),
    ]
    y = 4.30
...
# --- 2 本日の進め方 ---
s = slide_blank(); header(s, '本日の進め方', '目的と時間配分', '報告')
lead(s, [('■　本日は', 20, INK), ('【　　】分', 24, RED),
         ('。決裁2件・協議3件をこの場で決め切りたい', 20, INK)])
c2 = [(0.19, 4.00), (4.19, 9.00), (13.19, 3.09), (16.28, 11.00)]
for (x, w), t in zip(c2, ['時　間', '内　容', '区　分', 'この場で決めたいこと']):
    plain(s, x, 4.10, w, 1.00, [[(t, 16, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
agenda = [('【　　】分', 'Ⅰ. 結論', '決裁', '予算総額・人員・手数料還元率の承認'),
          ('【　　】分', 'Ⅱ. 戦略', '協議', '総名簿数の目標値をいくつに置くか'),
          ('【　　】分', 'Ⅲ. 施策', '協議', '4施策の着手順と担当'),
          ('【　　】分', 'Ⅳ. 数値', '決裁', '採用ケースと撤退基準'),
          ('【　　】分', 'Ⅴ. 実行', '協議', '初回セミナーの時期と体制')]
y = 5.10
for tm, ct, kd, goal in agenda:
    plain(s, 0.19, y, 4.00, 1.70, [[(tm, 16, RED)]], fill=PALE, line=NAVY, lw=1.0)
    plain(s, 4.19, y, 9.00, 1.70, [[(ct, 18, NAVY)]], fill=WHITE, line=NAVY, lw=1.0,
          align=PP_ALIGN.LEFT)
    bg, fg = KIND[kd]
    plain(s, 13.19, y, 3.09, 1.70, [[(kd, 16, fg)]], fill=bg, line=NAVY, lw=1.0)
    plain(s, 16.28, y, 11.00, 1.70, [[(goal, 16, INK)]], fill=WHITE, line=NAVY, lw=1.0,
          align=PP_ALIGN.LEFT)
    y += 1.70
plain(s, LM, 13.80, CW, 1.40,
      [[('本日いただきたい結論：', 20, INK), ('【　　　　　　　　　　　　】', 20, RED)]],
      fill=PALE, lw=1.5)
note(s, '起案者', '所要時間の配分、出席者', '—')
issue(s, '論点｜', '時間配分と、決裁事項の優先順位')
notes(s, '冒頭で「本日決めること」を握る。区分＝決裁の2件を必ず時間内に処理する。')
...
# --- 5 決裁依頼事項 ---
s = slide_blank(); header(s, 'Ⅰ. 結論', '決裁依頼事項', '決裁')
lead(s, [('■　ご承認いただきたいのは、次の', 20, INK), ('3点', 24, RED), ('です', 20, INK)])
c5 = [(0.19, 6.00), (6.19, 11.50), (17.69, 5.50), (23.19, 4.09)]
for (x, w), t in zip(c5, ['承認項目', '内　容', '金額・数値', '承認欄']):
    plain(s, x, 4.10, w, 1.10, [[(t, 16, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
y = 5.20
for a, b, c in [('① 予算総額', '広告費／ハガキ発送費／セミナー運営費／ツール費', '【　　　】万円'),
    ...
    plain(s, 23.19, y, 4.09, 2.40, [[('可 ・ 否 ・ 保留', 14, INK)]], fill=WHITE,
          line=NAVY, lw=1.0)
    y += 2.40
...
issue(s, '論点｜', '3点それぞれの承認可否と付帯条件')
notes(s, '投資総額＝固定費＋変動費。第17章の損益分岐点と数字を必ず一致させる。'
         '承認欄はその場で丸を付けていただく想定。主担当：役員。')
...
notes(s, 'KGI（粗利）→ 中間KPI（総名簿数）→ 施策KPI の3階層。各率は第14章の前提条件表と一致させる。主担当：経営企画。')
...
notes(s, '獲得→育成ハブ→刈り取り→名簿回収。各段の転換率は第14章の前提条件表で管理。主担当：起案者。')
...
card(s, LM, 4.05, CW, 1.30, 2.40, [('獲得名簿数の公式', 20, YEL)],
     [[('獲得名簿数 ＝ 提携社数', 20, INK), ('【　　】社', 20, RED),
       (' × 1社あたり平均名簿数', 20, INK), ('【　　　】件', 20, RED)],
      [('× 名簿提出率', 20, INK), ('【　】％', 20, RED)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, lw=2.5)
for x, hd, lines in [(LM, '① 無料点検を提示', ['提携先の顧客に', '無料点検を案内する', '大義名分をつくる']),
...
# --- 14 前提条件表 ---
s = slide_blank(); header(s, 'Ⅳ. 数値', '前提条件表', '協議')
lead(s, [('■　すべての数字に、', 20, INK), ('根拠区分', 24, RED), ('を明記する', 20, INK)])
c14 = [(0.19, 7.50), (7.69, 4.50), (12.19, 7.00), (19.19, 8.09)]
for (x, w), t in zip(c14, ['変　数', '仮説値', '根拠区分', '出典・備考']):
    plain(s, x, 4.05, w, 1.00, [[(t, 16, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
y = 5.05
for v in ['訪問数（月）', '面会率', '商談率', '提携率', 'クリック率（広告）', 'DL率',
          'メルマガ反応率', 'セミナー参加率', '名簿提出率', '1社あたり平均名簿数']:
...
for (x, w), t in zip(c15, ['項　目', '保守ケース', '標準ケース', '強気ケース']):
...
notes(s, '年間手数料 = 名簿数 × 点検実施率 × 手数料単価／未確定：メンテ手数料率、点検実施率。'
         'ここが弱いと第10章・第12章の提携率の前提が崩れる。主担当：営業。')

# --- 17 損益分岐点・回収期間 ---
s = slide_blank(); header(s, 'Ⅳ. 数値', '損益分岐点・回収期間', '決裁')
...
for x, hd, val in [(LM, 'BEP提携社数', '【　　】社'), (9.47, '投資回収期間', '【　　】ヶ月'),
                   (18.74, '撤退基準', '【　　】ヶ月時点で\n提携【　】社未満')]:
...
for x, hd, lines in [(0.19, '1ヶ月目', ['訪問開始', 'DL第1号を制作']),
...
for x, hd, val in [(0.19, '訪問数', '【　　】件'), (7.13, '獲得リード', '【　　　】件'),
...
notes(s, '月次KPI目標は第14章の前提条件表から逆算して設定する。主担当：起案者。')

# --- 19 体制・役割分担 ---
s = slide_blank(); header(s, 'Ⅴ. 実行', '体制・役割分担', '決裁')
lead(s, [('■　', 20, INK), ('増員なしで回るか', 24, RED), ('、ここで見極める', 20, INK)])
c19 = [(0.19, 5.50), (5.69, 4.50), (10.19, 12.00), (22.19, 5.09)]
for (x, w), t in zip(c19, ['機　能', '担　当', '主な業務', '工数（人月）']):
    plain(s, x, 4.10, w, 1.10, [[(t, 16, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
y = 5.20
for a, b, c in [('訪問営業', '◯◯', '工務店・不動産への訪問／提携交渉／名簿回収'),
...
issue(s, '論点｜', '増員するか、外部に委託するか')
notes(s, '訪問・マーケ・事務代行（ハガキ発送）の担当と工数。増員要否は役員判断。主担当：役員。')

# --- 20 リスクと対策 ---
s = slide_blank(); header(s, 'Ⅴ. 実行', 'リスクと対策', '協議')
...
issue(s, '論点｜', '法務レビューの依頼先と回答期限')
notes(s, '①個人情報：第三者提供と委託構成の切り分け。②非競合の契約条項化。'
         '法務レビューの結論待ち。主担当：法務。')

# --- 21 補足資料 ---
s = slide_blank(); header(s, 'Appendix', '補足資料', '報告')
lead(s, [('■　本編に添付する資料は', 20, INK), ('4点', 24, RED)])
for x, hd, lines in [(0.19, '① DLレポート案', ['「メンテでストック', '収入を作る協業モデル」']),
...
issue(s, '論点｜', '別紙の完成期限と担当の割り当て')
notes(s, 'DLレポート案・訪問トークスクリプト・提携契約書ドラフト・ハガキ原稿案。主担当：各担当。')

# --- 22 決定事項・宿題（記入欄） ---
s = slide_blank(); header(s, 'まとめ', '決定事項・宿題', '記録')
lead(s, [('■　本日の決定事項と宿題を、この場で', 20, INK), ('記入', 24, RED),
         ('して持ち帰る', 20, INK)])
c22 = [(0.19, 2.50), (2.69, 15.50), (18.19, 4.00), (22.19, 5.09)]
for (x, w), t in zip(c22, ['No.', '決定事項 ・ 宿題', '担　当', '期　限']):
    plain(s, x, 4.10, w, 1.00, [[(t, 16, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
y = 5.10
for i in range(1, 8):
    plain(s, 0.19, y, 2.50, 1.40, [[(str(i), 16, NAVY)]], fill=PALE, line=NAVY, lw=1.0)
    for x, w in c22[1:]:
        plain(s, x, y, w, 1.40, [[('', 16, INK)]], fill=WHITE, line=NAVY, lw=1.0)
    y += 1.40
note(s, '起案者', '本日の議事（この場で記入）', '—')
issue(s, '次回打合せ｜', '【　　月　　日】　次回までの宿題は上記のとおり')
notes(s, '打合せ中に記入する欄。終了時に読み上げて合意を取り、そのまま議事メモとして配布する。')
