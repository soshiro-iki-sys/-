...
def seminar_page(tag, lead_parts, sub, title, form, program, exit_, sched,
    ...
    plain(s, M, 12.00, CW, 0.90,
          [[('集客の逆算スケジュール　―　申込 13社 → 参加 9社 → 提携 1.5社 が目標',
...
plain(s, M, 12.35, CW, CB - 12.35,
      [[('この企画で狙うのは粗利', T_SUM, INK), ('2,060万円', T_SUM, RED), ('。', T_SUM, INK)],
...
notes(s, '施策ごとに「やること」と「追うKPI」を並べ、下段でそれが提携社数 → 名簿 → 粗利に'
         'つながることを示すページ。訪問6社＋フォーム/DL 4社＋セミナー6社＝提携16社。'
...
for _yr, _iv, _gp, _rt, _last in [('1年目', '452万円', '2,060万円', '4.6倍', False),
                                  ('2年目', '452万円', '4,440万円', '9.8倍', False),
                                  ('3年目', '452万円', '5,330万円', '11.8倍', False),
                                  ('3年累計', '1,356万円', '11,830万円', '8.7倍', True)]:
    _fl = PALE if _last else WHITE
