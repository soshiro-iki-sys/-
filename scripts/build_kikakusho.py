...
plain(s, LM, 11.20, CW, 4.00,
      [[('1件ずつ獲るのをやめ、', 28, INK)],
       [('1,650件', 28, RED), ('をまとめて獲りにいく', 28, INK)]], fill=PALE)
...
card(s, LM, 4.05, CW, 1.30, 2.40, [('獲得リード数の公式', 20, YEL)],
     [[('フォーム営業 1,920社 × 年6回（約11,200通）→ 累積DL率', 20, INK),
       ('7.0％', 20, RED), (' → ', 20, INK), ('134社', 20, RED)],
      [('＋ FB広告 月30万円 × 12ヶ月 → ', 20, INK), ('250件', 20, RED),
       ('　＝　獲得リード ', 20, INK), ('384件', 20, RED)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, body_spc=1.2, lw=2.5)
card(s, 0.29, 7.95, 13.17, 1.40, 3.75, [('新規網羅｜フォーム営業', 20, WHITE)],
     [[('・商圏3,000社 − 訪問先1,080社 ＝ 1,920社', 16, INK)],
      [('・2ヶ月に1本のDL資料を年6回配信', 16, INK)],
      [('・訪問先は除外し、営業と衝突させない', 16, INK)],
      [('・セールスブレイカー 月6万円（年72万円）', 16, INK)]], body_size=16)
card(s, 14.06, 7.95, 13.17, 1.40, 3.75, [('補完｜FB広告 ＋ メルマガ', 20, WHITE)],
     [[('・FB広告 月30万円 → 年250件', 16, INK)],
      [('・商圏外・リスト外の企業も拾う', 16, INK)],
      [('・DL者はメルマガへ自動登録', 16, INK)],
      [('・訪問先の名刺もメルマガへ合流', 16, INK)]], body_size=16)
plain(s, LM, 13.30, CW, 1.90,
      [[('人力で回れない1,920社に、', 24, INK), ('ツールで確実に届ける。', 24, RED)]],
      fill=PALE)
note(s, 'マーケ', 'フォーム営業のDL率、ツール割引の適用期間')
issue(s, '論点｜', '広告予算をいくらまで認めるか')
notes(s, '獲得リード数 = フォーム営業134社 ＋ FB広告250件 ＝ 384件。'
         'フォーム営業の母数は商圏3,000社から訪問先1,080社を除いた1,920社。主担当：マーケ。')
...
card(s, LM, 9.05, CW, 1.30, 2.40, [('提携社数の公式', 20, YEL)],
     [[('提携社数 ＝ 期中平均リスト', 20, INK), ('624件', 20, RED),
       (' × 申込率', 20, INK), ('2％', 20, RED),
       (' × 参加率', 20, INK), ('70％', 20, RED)],
      [('× 年4回 × 提携率', 20, INK), ('20％', 20, RED), ('　＝　', 20, INK),
       ('5社', 20, RED)]],
     body_fill=WHITE, body_align=PP_ALIGN.CENTER, lw=2.5)
plain(s, LM, 12.85, CW, 2.35,
      [[('前提：セミナーは', 22, INK), ('DM単独でも9社集客できる企画', 22, RED),
        ('にする。', 22, INK)],
       [('テレアポはその上積み。集客をテレアポに依存させない。', 22, INK)]],
      fill=PALE, spc=1.2)
note(s, 'マーケ／営業', 'セミナー申込率2％の実測')
...
groups = [
    ...
    ('② DL・フォーム営業',
     [('配信母数（年6回）', '1,920社', '商圏3,000社 − 訪問先1,080社'),
      ('累積DL率', '7.0％', '年6回で134社がダウンロード'),
      ('FB広告リード', '250件', '月30万円 × 12ヶ月')]),
    ('③ メルマガ・セミナー',
     [('メルマガ開封率', '16.9％', '船井総研：7,775通 → 1,313件'),
      ('セミナー申込率', '2％', '期中平均リスト624件 → 参加9社/回'),
...
y = 8.97
for a, vals in [('提携社数', ('16社', '24社', '32社')),
                ('総名簿数', ('1,100件', '1,650件', '2,200件')),
                ('アポ率', ('9％', '12％', '15％')),
                ('成約率', ('22％', '30％', '35％')),
                ('契約件数', ('14件', '41件', '83件')),
                ('売上', ('3,100万円', '1億400万円', '2億2,500万円')),
                ('粗利（KGI）', ('700万円', '2,600万円', '5,800万円'))]:
...
for x, hd, val in [(LM, '訪問数', '90社 /月'), (9.47, '獲得リード', '32件 /月'),
                   (18.74, '提携社数', '2社 /月')]:
...
exec_page('実行｜DLレポート',
          ...
          ['DL資料の企画・原稿作成（2ヶ月に1本・年6本）',
           'セールスブレイカーで訪問先を除いた1,920社へ配信',
           'FB広告の出稿（月30万円）',
           'ダウンロードページの用意と計測',
           'DL者のメルマガ自動登録とスコアリング'],
          'フォーム配信 960通　→　リード 32件 /月',
