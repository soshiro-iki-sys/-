...
def exec_page(tag, lead_parts, items, target, message, owner):
    ...
    for i, it in enumerate(items, start=1):
        ...
        plain(s, 1.99, y, 13.80, rh, [[(it, ts, INK)]], fill=WHITE, line=NAVY, lw=1.0,
              align=PP_ALIGN.LEFT)
        for x, w in cols[2:]:
            plain(s, x, y, w, rh, [[('', 16, INK)]], fill=WHITE, line=NAVY, lw=1.0)
        y += rh
...
exec_page('実行｜フォーム・DL',
          [('■　DL資料を、', 20, INK), ('何を作り、どう配るか', 24, RED),
           ('まで落とす', 20, INK)],
          ['リストDLで商圏3,000社を抽出し、業種で精査する',
           'DL資料の年間ラインナップを決める（年6本）',
           '第1号を制作する（工務店向け・提携モデルの解説）',
           'DL申込LPとサンクスメールを用意する（広告の受け皿）',
           'フォーム送信を配信する（月平均1,460通）',
           'FB広告でDLを訴求する（月30万円・LPへ誘導）'],
          'フォーム 18件　＋　FB広告 21件　＝　DL 38件 /月',
...
exec_page('実行｜メルマガ',
          ...
          ['配信リストをCRMに一本化する（訪問名刺・DL者）',
           ...
           '事例ストックを作る（営業から月2本吸い上げる）',
           '毎週配信し、開封・クリックを記録する（月2,900通）',
           'クリックした先を週次で営業へ渡す'],
...
exec_page('実行｜セミナー',
          [('■　年4回のセミナーを、', 20, INK), ('実行単位', 24, RED),
           ('まで落とす', 20, INK)],
          ['年間開催計画を確定する（年4回・会場を先に押さえる）',
           '企画書と当日コンテンツを作る（開催2ヶ月前まで）',
           '告知を開始する（1.5ヶ月前・メルマガ週1＋DM）',
           '申込者・反応者へ架電し、参加を後押しする',
           '当日運営（個別試算を提示し、社長同席で提携打診）',
           '翌営業日にお礼と個別面談の日程調整を送る'],
          '1回あたり 申込 13社　→　参加 9社　→　提携 1.5社（年6社）',
          'セミナーは四半期に1回。開催日から逆算して動く。', 'マーケ／営業')

# --- 21 次回勉強会の企画 ---
s = slide_blank(); header(s, 'Ⅴ. 実行', '次回勉強会の企画')
lead(s, [('■　次回の勉強会を、', 20, INK), ('企画・集客・出口', 24, RED),
         ('まで1枚に落とす', 20, INK)])
for x, hd, lines in [
        (LM, '開催概要',
         ['・開催日【　　月　　日】', '・会　場【　　　　　　　】',
          '・定　員 20社（1社2名まで）', '・対　象 工務店・リフォーム',
          '・参加費 無料']),
        (9.47, '当日プログラム（2時間）',
         ['・第1部 市場と補助金の最新', '・第2部 提携モデルの解説',
          '・第3部 個別の収益試算', '・終了後 個別相談の時間を取る',
          '・登壇【　　　　　　　　】']),
        (18.74, '出口設計',
         ['・参加全社に個別試算を提示', '・社長同席で決裁者に説明する',
          '・その場で締結書を渡す', '・翌営業日にお礼と日程調整',
          '・参加9社 → 提携1.5社が目標'])]:
    card(s, x, 4.05, 8.54, 1.30, 4.20, [(hd, 18, WHITE)],
         [[(t, 13, INK)] for t in lines], body_align=PP_ALIGN.LEFT, body_spc=1.45,
         body_size=13)
plain(s, LM, 9.85, CW, 1.00,
      [[('集客の逆算スケジュール　―　申込 13社 → 参加 9社 が目標', 16, WHITE)]],
      fill=NAVY, line=NAVY, lw=1.0)
for i, (hd, lines) in enumerate([
        ('1.5ヶ月前｜告知', ['・メルマガで初告知', '・DMを発送する', '・申込フォーム公開']),
        ('1ヶ月前｜追込み', ['・反応者へ架電', '・事務員と接点作り', '・メルマガ2回目']),
        ('2週間前｜確定', ['・受講案内を送付', '・欠席防止の連絡', '・当日資料を確定']),
        ('前日｜最終確認', ['・参加者リスト確定', '・個別提案先を選ぶ', '・社長同席の段取り']),
        ('当日｜刈り取り', ['・収益試算を提示', '・その場で提携打診', '・締結書を渡す'])]):
    x = 0.19 + i * 5.42
    plain(s, x, 10.85, 5.30, 1.00, [[(hd, 14, WHITE)]], fill=NAVY, line=NAVY, lw=1.0)
    plain(s, x, 11.85, 5.30, 3.00, [[(t, 12, INK)] for t in lines],
          fill=PALE, line=NAVY, lw=1.0, align=PP_ALIGN.LEFT, spc=1.5)
rec('マーケ／営業', '開催日・会場・登壇者・DM発送先')
keymsg(s, '集客は1.5ヶ月前から。ここが遅れると当日が埋まらない。')
notes(s, '次回勉強会の企画シート。T社＠山梨県の事例では集客期間は遅くとも当日1ヶ月半前から'
         '動いており、事務員・担当者と関係をつくることが参加率を左右している。'
         '当日は社長同席のもと決裁者へ提携の詳細を伝え、その場で締結書をもらう。'
         '開催日・会場・登壇者は打合せで確定する。主担当：マーケ／営業。')
