...
lead(s, [('■　商圏3,000社をツールで抽出し、訪問で回れない', 20, INK),
         ('1,920社', 24, RED), ('に届ける', 20, INK)])
card(s, LM, 4.05, CW, 1.30, 2.40, [('獲得リード数の公式', 20, YEL)],
     [[('フォーム送信 1,920社 × 年6回（約11,200通）→ 累積DL率', 20, INK),
...
card(s, 0.29, 7.95, 13.17, 1.40, 3.75, [('新規網羅｜フォーム送信', 20, WHITE)],
     [[('・リストDLで商圏3,000社を抽出', 16, INK)],
      [('・訪問先1,080社を除いた1,920社へ配信', 16, INK)],
      [('・2ヶ月に1本のDL資料を年6回', 16, INK)],
      [('・月平均960通（プラン上限3,000社の31％）', 16, INK)]], body_size=16)
card(s, 14.06, 7.95, 13.17, 1.40, 3.75, [('育成｜メルマガ ＋ FB広告', 20, WHITE)],
     [[('・DL者と訪問名刺をリストに積み上げる', 16, INK)],
      [('・期中平均624件へ週1配信', 16, INK)],
      [('・月2,500通（プラン上限15,000通の17％）', 16, INK)],
      [('・FB広告 月30万円 → 年250件を上乗せ', 16, INK)]], body_size=16)
plain(s, LM, 13.30, CW, 1.90,
      [[('人力で回れない1,920社にツールで届け、', 24, INK),
        ('DL者をメルマガで育てる。', 24, RED)]], fill=PALE)
...
groups = [
    ...
    ('② フォーム・メルマガ・DL',
     [('フォーム配信母数', '1,920社', '商圏3,000社 − 訪問先1,080社'),
      ('累積DL率（年6回）', '7.0％', '134社がDL。月960通＝プラン枠の31％'),
...
exec_page('実行｜フォーム・DL',
          [('■　DL資料を、', 20, INK), ('何を作り、どう配るか', 24, RED),
           ('まで落とす', 20, INK)],
          ['リストDLで商圏3,000社を抽出し、訪問先を除外',
           'DL資料の企画・原稿作成（2ヶ月に1本・年6本）',
           'フォーム送信で1,920社へ配信（月平均960通）',
           'FB広告の出稿（月30万円）',
           'DL者のメルマガ自動登録と週1配信（月2,500通）'],
